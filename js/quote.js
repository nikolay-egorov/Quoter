...
function drawImageScaled(  x, y, fullwidth) {
    ...
    var hRatio = this.w_sample / this.width;
...
function buildCollage() {
    ...
    var numImages = 5;
    var src = "https://source.unsplash.com/400x300?sig=";
    var x = 0, y = 0;
    for (var i = 0; i < numImages; i++) {
        if (x < globalWidth - 4) {
            drawImgFromUnsplash(src+ Math.random(), x, y, 0);
            x += w_sample;
        } else {
            x = 0;
            y += h_sample;
            if (i === numImages -1 ) {
                ...
                //     }));
                drawImgFromUnsplash(src+ Math.random(), x, y, 1);
